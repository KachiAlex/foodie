...
import { ReviewModal } from "@/pages/community/ReviewModal";
                ...
                <div className="rounded-2xl bg-[#1a1d27] border border-white/8 p-6 shadow-sm">
                  <div className="flex items-center justify-between">
                    ...
                    <Button variant="ghost" size="sm" className="text-xs text-orange-600" asChild>
                      <Link to="/community/vendor-market">View all</Link>
                    </Button>
                  </div>
                  <div className="mt-4 space-y-3">
          ...
          <div className="space-y-4">
            ...
            {orders.length === 0 ? (
              <div className="flex flex-col items-center justify-center rounded-3xl border-2 border-dashed border-white/10 py-20 text-center">
                ...
                <Button className="mt-6 bg-orange-500" onClick={() => setShowNewRequest(true)}>
              ...
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                {orders.map((order) => (
                  <motion.div
                    ...
                    className="rounded-3xl bg-[#1a1d27] border border-white/8 p-5 space-y-4"
                  >
                    <div className="flex justify-between items-start">
                      <div>
                        <h3 className="font-bold text-white text-lg">{order.chef}</h3>
                        <p className="text-xs text-gray-400">{new Date(order.createdAt ?? "").toLocaleDateString()}</p>
                      </div>
                      <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest ${
                        order.status === 'Delivered' ? 'bg-emerald-500/20 text-emerald-400' :
                        order.status === 'Cooking' ? 'bg-orange-500/20 text-orange-400' :
                    ...
                    <div className="p-3 rounded-2xl bg-white/5 border border-white/5">
                      <p className="text-xs text-gray-500 uppercase font-bold tracking-tighter">Meal Items</p>
                      <p className="text-sm text-gray-200 mt-1 font-medium">{order.dishes}</p>
                    </div>

                    <div className="flex justify-between items-center pt-2">
                      <div className="text-2xl font-black text-white">
                        {symbol}{order.amount.toLocaleString()}
                      </div>
                      ...
                      {order.status === "Delivered" && (
                        order.review ? (
                          <div className="flex items-center gap-1 text-yellow-500 bg-yellow-500/10 px-3 py-1.5 rounded-xl border border-yellow-500/20">
                            ...
                            <span className="text-xs font-bold">{order.review.rating}</span>
                          </div>
                        ) : (
                          <Button 
                            size="sm" 
                            className="bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl font-bold gap-2"
                            onClick={() => setReviewOrderId(order.id)}
                          >
                            <Star className="h-4 w-4 fill-white" /> Rate Meal
                          </Button>
                        )
                      )}
                    </div>

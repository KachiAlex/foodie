...
export async function uploadVendorDocument(type: string, file: File): Promise<UploadedDocument> {
  const fileBase64 = await fileToDataUrl(file);
  return api.post<UploadedDocument>("/vendors/documents", { type, fileBase64 });
}

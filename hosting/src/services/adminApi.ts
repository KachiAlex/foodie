...
export async function resolveDispute(disputeId: string, resolution: string): Promise<ActionResponse> {
  const data = await api.patch<ActionResponse>(`/admin/disputes/${disputeId}/resolve`, { resolution });
  return data;
}
...
export async function getVendorDocuments(vendorId: string): Promise<PendingVendor> {
  const data = await api.get<PendingVendor>(`/admin/vendors/${vendorId}/documents`);
  return data;
}

export async function approveDocument(documentId: string): Promise<VendorDocument> {
  const data = await api.patch<VendorDocument>(`/admin/documents/${documentId}/approve`, {});
  return data;
}

export async function rejectDocument(documentId: string, reason?: string): Promise<VendorDocument> {
  const data = await api.patch<VendorDocument>(`/admin/documents/${documentId}/reject`, { reason });
  return data;
}
